...
from pipeline.training.training_loop import HRLTrainingLoop
...
class FlatTrainingLoop(HRLTrainingLoop):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self._state_representation == "dialogue_act":
            ...
            try:
                ...
                from pipeline.flat_rl.env_dialogue_act import FlatDialogueActEnv
        ...
        self.agent = FlatActorCriticAgent(
            ...
            use_lstm=True,
            device=self.device,
            sampling_temperature=float(os.environ.get("HRL_POLICY_TEMPERATURE", "1.0")),
        )
    ...
    def _run_episode(self):
        ...
        while turn_count < self.max_turns_per_episode:
            ...
            # Use one coherent snapshot for both neural-network inference and
            # action masking.  With the fixed transition timing this must equal
            # the previous turn's post-simulator next state.
            ...
            action_mask = self.env.get_flat_action_mask()
            ...
            next_obs, reward, done, truncated, info = self.env.step(
                flat_action, post_action_callback=self._update_simulator_state
            )
            ...
            timing_validation = {
                ...
                "selected_action": info.get("flat_action_name"),
            ...
            if self.detailed_logger:
                ...
                action_for_log = {
                    ...
                    "sampling_temperature": action_dict.get("sampling_temperature"),
                    "selected_action_probability": action_dict.get("selected_action_probability"),
                    "state_value": action_dict.get("state_value"),
                    "action_logits": action_dict.get("action_logits"),
                    "action_probabilities": action_dict.get("action_probabilities"),
                }
