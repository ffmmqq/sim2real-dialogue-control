...
from pipeline.utils.dialogue_planner import build_prompt 
from pipeline.utils.knowledge_graph import SimpleKnowledgeGraph
from pipeline.utils.dialoguebert_intent_recognizer import get_dialoguebert_recognizer
...
class MuseumDialogueEnv(gym.Env):
    ...
    def __init__(self, deliberation_cost=0.0, knowledge_graph_path=None, max_turns=20, 
        ...
        projection_rng = np.random.RandomState(42)
        self.projection_matrix = projection_rng.randn(64, 768).astype(np.float32) / np.sqrt(768)
    ...
    def step(
        self,
        action_dict,
        post_action_callback: Optional[Callable[[Dict[str, Any]], Optional[Dict[str, Any]]]] = None,
    ):
        """Execute one state-synchronised step in the SMDP environment.

        ``post_action_callback`` is the simulator boundary.  The guide action and
        its coverage changes happen first; the callback then produces the current
        visitor response and updates utterance/dwell/focus.  Reward calculation
        and the returned observation happen only after that update, so the TD
        transition is ``(s_t, a_t, r_t, s_{t+1})`` from one coherent turn.
        """
        current_exhibit_before_step = self._get_current_exhibit()
        coverage_before_step = self._get_museum_exhibit_coverage()
        ...
        for fact_id in fact_ids_mentioned:
            ...
            if (
                fact_id not in self.facts_mentioned_per_exhibit[ex_id]
                and fact_id not in new_fact_ids
            ):
                new_fact_ids.append(fact_id)
                if verbose:
                    print(f"✅ NEW FACT PENDING SIMULATOR: [{fact_id}]")
            else:
                if verbose:
        ...
        all_exhibits_complete = all(
            (
                cov_data["mentioned"]
                + (len(new_fact_ids) if exhibit_name == ex_id else 0)
            ) >= cov_data["total"]
            for exhibit_name, cov_data in coverage.items()
        ...
        if all_exhibits_complete and option != "Conclude":  # Don't override if already concluded manually
            ...
            from pipeline.utils.dialogue_planner import build_wrap_up_prompt
            
            # Get context for conclusion
            all_facts = self.knowledge_graph.get_exhibit_facts(ex_id)
            mentioned_after_action = self.facts_mentioned_per_exhibit[ex_id] | set(new_fact_ids)
            mentioned_facts = [
                fact for fact in all_facts
                if self.knowledge_graph.extract_fact_id(fact) in mentioned_after_action
            ]
            ...
            if os.environ.get('HRL_TEMPLATE_MODE') == '1':
                ...
                from pipeline.LLM_CONFIG import get_agent_llm
                llm = get_agent_llm()
...
Thank the visitor for exploring all exhibits. Keep it warm and brief (2-3 sentences)."""
        ...
        valid_ids_after_action = self.facts_mentioned_per_exhibit[ex_id] | set(new_fact_ids)
        exhibit_exhausted_initial = valid_fact_ids.issubset(valid_ids_after_action)

        # Capture the exact legacy-timing state for validation, then cross the
        # simulator boundary before reward calculation or next-state creation.
        # This makes it impossible for callers to store a pre-simulator state as
        # s_(t+1) while using a post-simulator action mask on the next turn.
        state_next_before_simulator = self._get_obs()
        current_exhibit_completion_for_simulator = (
            (len(self.facts_mentioned_per_exhibit[ex_id]) + len(new_fact_ids)) /
            len(self.knowledge_graph.get_exhibit_facts(ex_id))
            if self.knowledge_graph.get_exhibit_facts(ex_id) else 0.0
        )
        target_exhibit_completion_for_simulator = 0.0
        target_exhibit_exhausted_for_simulator = False
        if target_exhibit and target_exhibit in self.exhibit_keys:
            ...
            target_exhibit_completion_for_simulator = (
                (
                    len(self.facts_mentioned_per_exhibit[target_exhibit])
                    + (len(new_fact_ids) if target_exhibit == ex_id else 0)
                ) / len(target_facts)
                if target_facts else 0.0
            )
            target_exhibit_exhausted_for_simulator = self._is_exhibit_exhausted(target_exhibit)

        simulator_response: Dict[str, Any] = {}
        if post_action_callback is not None:
            callback_result = post_action_callback({
                "agent_utterance": agent_response,
                "option": "Conclude" if auto_concluded else option,
                "subaction": "WrapUp" if auto_concluded else subaction,
                "target_exhibit": target_exhibit,
                "current_exhibit": ex_id,
                "current_exhibit_completion": current_exhibit_completion_for_simulator,
                "exhibit_exhausted": exhibit_exhausted_initial,
                "target_exhibit_completion": target_exhibit_completion_for_simulator,
                "target_exhibit_exhausted": target_exhibit_exhausted_for_simulator,
                "auto_concluded": auto_concluded,
            })
            if callback_result:
                simulator_response = callback_result

        # Commit the current guide action's coverage only after the simulator
        # response has updated utterance/dwell/focus.  The callback received the
        # prospective completion above, preserving transition semantics.
        ...
        # Question-asking reward (hybrid design - H2).  The callback above has
        # already installed the current action's visitor response.
        ...
        # The authoritative next state is built only after the simulator has
        # updated the current visitor utterance, dwell, response type and focus.
        state_next_after_simulator = self._get_obs()
        current_exhibit_after_step = self._get_current_exhibit()
        coverage_after_step = self._get_museum_exhibit_coverage()
        ...
        info.update({
            "state_next_before_simulator": state_next_before_simulator.astype(float).tolist(),
            "state_next_after_simulator": state_next_after_simulator.astype(float).tolist(),
            "current_exhibit_before": current_exhibit_before_step,
            "current_exhibit_after": current_exhibit_after_step,
            "coverage_before": coverage_before_step,
            "coverage_after": coverage_after_step,
            "simulator_response": simulator_response,
            "transition_success": bool(simulator_response.get("transition_success", False)),
            "reward_response_utterance": self.last_user_utterance,
            "reward_response_type": self._last_user_response_type,
        })
        ...
        return state_next_after_simulator, step_reward, done, False, info
