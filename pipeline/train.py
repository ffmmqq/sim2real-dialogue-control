...
from pipeline.flat_rl.training_loop import FlatTrainingLoop
...
Examples:
  ...
  python train.py --episodes 20 --reward_mode baseline --name quick_test
    ...
    parser.add_argument('--checkpoint-interval', type=int, default=50,
                       help='Save trainer checkpoint every N episodes (default: 50)')
    ...
    parser.add_argument('--experiment-dir', type=str, default=None,
                       help='Write a fresh run to this exact experiment directory')
    parser.add_argument('--resume-checkpoint', type=str, default=None,
                       help='Path to trainer checkpoint .pt file for resuming training')
    parser.add_argument('--resume-metrics', type=str, default=None,
                       help='Path to checkpoint metrics JSON for resuming metrics state')
    parser.add_argument('--resume-experiment-dir', type=str, default=None,
                       help='Reuse an existing experiment directory instead of creating a new one')
    
    args = parser.parse_args()

    if args.experiment_dir and args.resume_experiment_dir:
        parser.error('--experiment-dir and --resume-experiment-dir are mutually exclusive')
    ...
    # Check for a Groq key only when Groq is the selected backend.
    if os.environ.get("HRL_LLM_BACKEND", "groq") == "groq" and "GROQ_API_KEY" not in os.environ:
    ...
    explicit_experiment_dir = args.experiment_dir or args.resume_experiment_dir
    if explicit_experiment_dir:
        exp_dir = Path(explicit_experiment_dir)
        exp_dir.mkdir(parents=True, exist_ok=True)
        for subdir in ["logs", "maps", "models", "checkpoints", "detailed_logs", "parameterization_results"]:
    ...
    metadata = {
        ...
        "gamma": args.gamma,
        "llm_backend": os.environ.get("HRL_LLM_BACKEND", "groq"),
        "llm_model": os.environ.get("HRL_LLM_MODEL", "llama-3.1-8b"),
    ...
    print("State: constructed by selected environment (151-dim for this H1 configuration)")
    print(
        f"LLM: {os.environ.get('HRL_LLM_BACKEND', 'groq')} / "
        f"{os.environ.get('HRL_LLM_MODEL', 'llama-3.1-8b')}"
    )
    print("=" * 80)
    ...
    training_loop = loop_cls(
        ...
        resume_metrics_path=args.resume_metrics
    )
    ...
    try:
        from pipeline.utils.parameterization_analyzer import ParameterizationAnalyzer
