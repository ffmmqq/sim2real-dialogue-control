...
from pipeline.visualization.training_monitor import TrainingMonitor
from pipeline.visualization.live_training_monitor import LiveTrainingMonitor
from pipeline.visualization.metrics_tracker import MetricsTracker
from pipeline.visualization.museum_map_visualizer import MuseumMapVisualizer
from pipeline.visualization.live_progress import LiveProgressTracker
from pipeline.agent.actor_critic_agent import ActorCriticAgent
from pipeline.training.actor_critic_trainer import ActorCriticTrainer
from pipeline.utils.detailed_logger import DetailedEpisodeLogger
...
class HRLTrainingLoop:
    ...
    def __init__(self, max_episodes: int = 10, max_turns_per_episode: int = 20,
        ...
        if experiment_dir:
            ...
            self.analytics_dir = Path(experiment_dir) / "analytics"
        ...
        self.resume_metrics_path = resume_metrics_path
    ...
    def run_training(self):
        ...
        print(f"Exhibits: {len(self.env.exhibit_keys)} | Checkpoints: Every {self.checkpoint_interval} episodes")
        ...
        try:
            ...
            for episode in range(self.current_episode, self.max_episodes):
                ...
                if self.current_episode % 10 == 0:
                    self._save_periodic_metrics_snapshot()
            ...
            from pipeline.utils.llm_handler import LLMCriticalError
    ...
    def _save_periodic_metrics_snapshot(self, force: bool = False):
        """Save every-10-episode action/exploration snapshots for downstream analysis."""
        if self.analytics_dir is None:
            return
        if not force and self.current_episode % 10 != 0:
            return

        window = 10
        recent_usage = self.metrics_tracker.episode_option_usage[-window:]
        window_counts = defaultdict(int)
        for usage in recent_usage:
            for action_name, count in usage.items():
                window_counts[action_name] += int(count)

        epsilon = None
        if hasattr(self, "agent") and hasattr(self.agent, "get_epsilon"):
            epsilon = self.agent.get_epsilon()

        policy_temperature = None
        if hasattr(self, "agent") and hasattr(self.agent, "get_sampling_temperature"):
            policy_temperature = self.agent.get_sampling_temperature()

        snapshot = {
            "episode": self.current_episode,
            "window_size_episodes": min(window, len(self.metrics_tracker.episode_option_usage)),
            "window_action_counts": dict(window_counts),
            "cumulative_action_counts": dict(getattr(self.metrics_tracker, "flat_action_counts", self.metrics_tracker.option_counts)),
            "epsilon": epsilon,
            "policy_temperature": policy_temperature,
        }

        snapshot_file = self.analytics_dir / f"snapshot_ep{self.current_episode:04d}.json"
        with open(snapshot_file, "w", encoding="utf-8") as f:
            json.dump(snapshot, f, indent=2)

        aggregate_file = self.analytics_dir / "periodic_snapshots.json"
        snapshots = []
        ...
        snapshots = [s for s in snapshots if s.get("episode") != self.current_episode]
        snapshots.append(snapshot)
        snapshots.sort(key=lambda item: item.get("episode", 0))
    ...
    def _run_episode(self) -> tuple:
        ...
        session_info = self.simulator.initialize_session(persona="Agreeable") or {}
        ...
        if self.detailed_logger:
            self.detailed_logger.start_episode(
                self.current_episode,
                profile=session_info.get("persona_profile", getattr(self.simulator, "persona_profile", None)),
                persona=session_info.get("persona", getattr(self.simulator, "current_persona", None)),
                starting_exhibit=session_info.get("exhibit", getattr(self.simulator, "current_exhibit", None)),
            )
        ...
        if self.current_episode % 10 == 0 or self.current_episode == 1:
            from pipeline.utils.llm_handler import get_llm_handler
            llm = get_llm_handler()
        ...
        while turn_count < self.max_turns_per_episode:
            ...
            temporal_state_match = states_equal(obs, current_state)
            obs = current_state
            current_exhibit_before = self.env._get_current_exhibit()
            coverage_before = self.env._get_museum_exhibit_coverage()
            before_validation = validate_state(
                self.env, obs, current_exhibit_before, coverage_before
            )
            action_mask = self.env.get_action_masks()
            mask_state_match = states_equal(obs, self.env._get_obs())
            ...
            next_obs, reward, done, truncated, info = self.env.step(
                action, post_action_callback=self._update_simulator_state
            )
            episode_timing["env_step_time"] += time.time() - _step_start
            after_validation = validate_state(
                self.env,
                next_obs,
                info["current_exhibit_after"],
                info["coverage_after"],
            )
            current_response = info.get("simulator_response", {})
            current_gaze = current_response.get("gaze_features") or []
            current_response_dwell = float(current_gaze[0]) if current_gaze else None
            reward_response_match = (
                (current_response_dwell is None or abs(float(info.get("dwell", 0.0)) - current_response_dwell) <= 1e-6)
                and info.get("reward_response_utterance", "") == current_response.get("utterance", "")
                and info.get("reward_response_type") == current_response.get("response_type", "statement")
            )
            timing_validation = {
                "state_before": obs.astype(float).tolist(),
                "state_next_before_simulator": info["state_next_before_simulator"],
                "state_next_after_simulator": info["state_next_after_simulator"],
                "current_exhibit_before": current_exhibit_before,
                "current_exhibit_after": info["current_exhibit_after"],
                "coverage_before": coverage_before,
                "coverage_after": info["coverage_after"],
                "focus_encoded_in_state": before_validation["focus_encoded_in_state"],
                "focus_encoded_in_next_state": after_validation["focus_encoded_in_state"],
                "coverage_encoded_in_state": before_validation["coverage_encoded_in_state"],
                "coverage_encoded_in_next_state": after_validation["coverage_encoded_in_state"],
                "action_mask": action_mask,
                "selected_action": {
                    "option": info.get("option"),
                    "subaction": info.get("subaction"),
                },
                "transition_success": info.get("transition_success", False),
                "reward_input_dwell": info.get("dwell"),
                "simulator_response_dwell": current_response_dwell,
                ...
                "state_exhibit_match": before_validation["exhibit_match"] and after_validation["exhibit_match"],
                "state_coverage_match": before_validation["coverage_match"] and after_validation["coverage_match"],
                "temporal_state_match": temporal_state_match,
                "action_mask_state_match": mask_state_match,
            }
            info["timing_validation"] = timing_validation
            if not all((
                timing_validation["state_exhibit_match"],
                timing_validation["state_coverage_match"],
                timing_validation["temporal_state_match"],
                timing_validation["action_mask_state_match"],
                timing_validation["reward_response_match"],
            )):
                raise RuntimeError(f"Turn-level state timing validation failed: {timing_validation}")
            ...
            # Simulator ran inside env.step before reward and next-state creation.
    ...
    def _update_simulator_state(self, info: Dict[str, Any]):
        """Generate and install the current action's simulator response."""
        agent_utterance = info.get("agent_utterance", "")
    ...
    def _maybe_resume_training_state(self):
        """Restore trainer + metrics state from a previous checkpoint if provided."""
        if self._resume_loaded or not self.resume_checkpoint_path:
            return
        ...
        restored_episode = self.trainer.load_checkpoint(str(checkpoint_path))
        self.current_episode = restored_episode
        print(f"[RESUME] Restored trainer state from {checkpoint_path.name} (episode {restored_episode})")

        if self.resume_metrics_path:
            metrics_path = Path(self.resume_metrics_path)
            if metrics_path.exists():
                self._restore_metrics_tracker(metrics_path)
                print(f"[RESUME] Restored metrics state from {metrics_path.name}")
            else:
                print(f"[WARNING] Resume metrics not found: {metrics_path}")

        self._resume_loaded = True

    def _restore_metrics_tracker(self, metrics_path: Path):
        """Load metrics tracker state from saved JSON checkpoint."""
        with open(metrics_path, "r", encoding="utf-8") as f:
            data = json.load(f)

        mt = self.metrics_tracker
        mt.episode_returns = data.get("episode_returns", [])
        mt.episode_lengths = data.get("episode_lengths", [])
        mt.episode_coverage = data.get("episode_coverage", [])
        mt.episode_facts = data.get("episode_facts", [])
        mt.episode_option_usage = data.get("episode_option_usage", [])
        mt.option_counts = defaultdict(int, data.get("option_counts", {}))
        mt.flat_action_counts = defaultdict(int, data.get("flat_action_counts", {}))
        mt.option_durations = defaultdict(list, data.get("option_durations", {}))
        mt.option_transitions = defaultdict(lambda: defaultdict(int))
        for src, dests in data.get("option_transitions", {}).items():
            mt.option_transitions[src] = defaultdict(int, dests)
        mt.episode_subaction_usage = data.get("episode_subaction_usage", [])
        mt.subaction_counts = defaultdict(int, data.get("subaction_counts", {}))
        mt.turn_subactions = data.get("turn_subactions", [])
        mt.option_entropy_per_episode = data.get("option_entropy_per_episode", [])
        mt.option_collapse_count = data.get("option_collapse_count", 0)
        mt.value_losses = data.get("value_losses", [])
        mt.policy_losses = data.get("policy_losses", [])
        mt.entropies = data.get("entropies", [])
        mt.value_estimates = data.get("value_estimates", [])
        mt.advantages = data.get("advantages", [])
        mt.termination_losses = data.get("termination_losses", [])
        mt.episode_termination_probs = data.get("episode_termination_probs", [])
        mt.episode_option_advantages = data.get("episode_option_advantages", [])
        mt.episode_intra_advantages = data.get("episode_intra_advantages", [])
        mt.episode_option_qvalues = data.get("episode_option_qvalues", [])
        mt.episode_option_switches = data.get("episode_option_switches", [])
        mt.episode_option_durations = data.get("episode_option_durations", [])
        mt.episode_per_option_advantages = data.get("episode_per_option_advantages", [])
        mt.episode_per_option_qvalues = data.get("episode_per_option_qvalues", [])
        mt.episode_action_advantages = data.get("episode_action_advantages", [])
        mt.episode_per_action_logits = data.get("episode_per_action_logits", [])
        mt.option_collapse_index = data.get("option_collapse_index", [])
        mt.policy_confidence = data.get("policy_confidence", [])
        mt.q_value_spread = data.get("q_value_spread", [])
        mt.exploration_rate = data.get("exploration_rate", [])
        mt.advantage_signs = data.get("advantage_signs", [])
        mt.termination_effectiveness = data.get("termination_effectiveness", [])
        mt.duration_stats = data.get("duration_stats", [])
        mt.current_episode_options = defaultdict(int)
        mt.current_episode_subactions = defaultdict(int)

        self.episode_rewards = list(mt.episode_returns)
    ...
    def _finalize_training(self):
        ...
        self._save_periodic_metrics_snapshot(force=True)
    ...
    def _organize_major_results(self, experiment_dir: str, training_time: float, timestamp: str):
        ...
        from pathlib import Path
        import json
        from pipeline.utils.major_results_manager import MajorResultsManager
        from pipeline.utils.major_results_templates import create_readme_file
        from pipeline.utils.metrics_validator import MetricsValidator
