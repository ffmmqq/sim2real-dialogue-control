...
def _build_history_vector(
    ...
    """
    history = np.zeros(n_exhibits + len(action_labels), dtype=np.float32)
    ...
    for i, action in enumerate(action_labels):
        history[n_exhibits + i] = action_counts.get(action, 0) / total_actions
...
def build_state(
    ...
    history = _build_history_vector(
        facts_mentioned, knowledge_graph, action_counts, n_exhibits, action_labels
    )
