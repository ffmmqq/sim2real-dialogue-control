...
from simulator.state_machine_simulator import (
    VisitorState,
    DWELL_RANGES,
    STATE_PROMPTS,
    FALLBACK_TEMPLATES,
...
class HybridSimulator:
    ...
    def __init__(
        ...
        self.seed = seed
        # Session scheduling must not consume the within-session behavior stream.
        # This keeps profile/exhibit order identical across learner-seed runs even
        # when their policies produce different trajectories and random draws.
    ...
    def initialize_session(
        ...
        self.current_persona = persona or self.schedule_rng.choice(self.PERSONAS)
        self.current_exhibit = self.schedule_rng.choice(self.exhibits)
        ...
        if persona_profile:
            ...
            self.persona_profile = self.schedule_rng.choice(list(self.PERSONA_PROFILES.keys()))
